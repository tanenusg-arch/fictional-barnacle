// =========================== FULL NODE.JS BOT (FIXED DB SCHEMA) ===========================
...
async function initDB() {
    ...
    // Use exec to run multiple CREATE TABLE statements
    await exec(`
...
// ====== FIXED safeEdit – handles media messages ======
async function safeEdit(ctx, newText, extra = {}) {
    try {
        const msg = ctx.callbackQuery?.message || ctx.message;
        if (!msg) {
            // fallback: send a new message if no message to edit
            await ctx.reply(newText, { parse_mode: 'HTML', ...extra });
            return;
        }
        // If the original message is a media (photo/video/document/animation), edit its caption
        ...
        // Ignore "Message is not modified" (common when content is identical)
        if (!e.message.includes('Message is not modified')) {
            console.error('safeEdit error:', e);
            // Optionally, notify the user
            try {
                await ctx.reply('⚠️ An error occurred while updating the message.');
            } catch (_) {}
        }
    }
}
...
(async () => {
    ...
    // 👇 FIX: Remove any existing webhook before starting polling
    try {
        await bot.telegram.setWebhook({ url: '' });
        console.log('Webhook removed successfully.');
    } catch (e) {
        console.warn('Failed to remove webhook:', e.message);
    }
