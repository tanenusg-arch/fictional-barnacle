// bot.js - Full Node.js Telegram Reseller Bot
// Requires: telegraf, sqlite3, axios, dotenv (optional)
// Run: node bot.js
...
const ADMIN_CHAT_IDS = config.ADMIN_CHAT_IDS || [];
...
const DB = {
    async addUser(telegram_id, username, first_name) {
        await db.run(`INSERT INTO users (telegram_id, username, first_name, registered_at)
    ...
    async banUser(telegram_id) {
        await db.run("UPDATE users SET banned = 1 WHERE telegram_id = ?", telegram_id);
        const row = await db.get("SELECT changes()");
        if (!row || row['changes()'] === 0) {
            await db.run("INSERT INTO users (telegram_id, username, first_name, balance, banned, registered_at) VALUES (?, '', '', 0.0, 1, datetime('now'))", telegram_id);
        }
    },
    async unbanUser(telegram_id) {
        await db.run("UPDATE users SET banned = 0 WHERE telegram_id = ?", telegram_id);
    },
    async updateBalance(telegram_id, amount) {
        await db.run("UPDATE users SET balance = balance + ? WHERE telegram_id = ?", amount, telegram_id);
    },
    async setBalance(telegram_id, amount) {
        await db.run("UPDATE users SET balance = ? WHERE telegram_id = ?", amount, telegram_id);
    },
    ...
    async updateReferralBalance(telegram_id, amount) {
        await db.run("UPDATE users SET referral_balance = referral_balance + ? WHERE telegram_id = ?", amount, telegram_id);
    },
    async canPlaceOrder(user_id) {
        const today = new Date().toISOString().slice(0,10);
        const row = await db.get("SELECT daily_order_count, last_order_date FROM users WHERE telegram_id = ?", user_id);
        if (!row) return true;
        if (row.last_order_date !== today) {
            await db.run("UPDATE users SET daily_order_count = 0, last_order_date = ? WHERE telegram_id = ?", today, user_id);
            return true;
        }
    ...
    async incrementOrderCount(user_id) {
        const today = new Date().toISOString().slice(0,10);
        await db.run("UPDATE users SET daily_order_count = daily_order_count + 1, last_order_date = ? WHERE telegram_id = ?", today, user_id);
    },
    async getPendingDeposits() {
        return db.all("SELECT * FROM deposits WHERE status = 'pending' ORDER BY id");
    },
    ...
    async approveDeposit(id, note = '') {
        ...
        await db.run(`UPDATE deposits SET status = 'approved', admin_note = ?, resolved_at = datetime('now') WHERE id = ?`, note, id);
    ...
    async rejectDeposit(id, reason = '') {
        const deposit = await this.getDepositById(id);
        if (!deposit || deposit.status !== 'pending') return false;
        await db.run(`UPDATE deposits SET status = 'rejected', admin_note = ?, resolved_at = datetime('now') WHERE id = ?`, reason, id);
        return true;
    },
    async createDeposit(user_id, method, amount, currency, proof_file_id) {
        const result = await db.run(`INSERT INTO deposits (user_id, method, amount, currency, proof_file_id, status, created_at)
                                     VALUES (?, ?, ?, ?, ?, 'pending', datetime('now'))`,
                                     user_id, method, amount, currency, proof_file_id);
        return result.lastID;
    },
    async getPendingWithdrawals() {
        return db.all("SELECT * FROM withdrawals WHERE status = 'pending' ORDER BY id");
    },
    ...
    async approveWithdrawal(id, note = '') {
        ...
        await db.run(`UPDATE withdrawals SET status = 'approved', admin_note = ?, resolved_at = datetime('now') WHERE id = ?`, note, id);
        return true;
    },
    async rejectWithdrawal(id, reason = '') {
        const w = await this.getWithdrawalById(id);
        if (!w || w.status !== 'pending') return false;
        await db.run(`UPDATE users SET balance = balance + ? WHERE telegram_id = ?`, w.amount, w.user_id);
        await db.run(`UPDATE withdrawals SET status = 'rejected', admin_note = ?, resolved_at = datetime('now') WHERE id = ?`, reason, id);
        return true;
    },
    async createWithdrawal(user_id, method, amount, currency, account, nickname, fee = 0) {
        await db.run("UPDATE users SET balance = balance - ? WHERE telegram_id = ?", amount, user_id);
        const result = await db.run(`INSERT INTO withdrawals (user_id, method, amount, currency, account, nickname, fee, status, created_at)
                                     VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', datetime('now'))`,
    ...
    async createOrder(telegram_id, order_id, status, game, service, player_id, nickname, package_name, api_price, charged_price, markup, api_response) {
        await db.run(`INSERT INTO orders (telegram_id, order_id, status, game, service, player_id, nickname, package_name, api_price, charged_price, markup_percent, api_response, created_at)
                      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))`,
    ...
    async getUserOrders(telegram_id, limit = 5) {
        return db.all("SELECT * FROM orders WHERE telegram_id = ? ORDER BY id DESC LIMIT ?", telegram_id, limit);
    },
    async getUserProfile(telegram_id) {
        const user = await this.getUser(telegram_id);
        if (!user) return null;
        const stats = await db.get("SELECT COUNT(*) as total_orders, SUM(charged_price) as total_spent FROM orders WHERE telegram_id = ? AND status = 'COMPLETED'", telegram_id);
        return {
            ...user,
    ...
    async getDashboardStats() {
        const total_users = (await db.get("SELECT COUNT(*) as cnt FROM users WHERE banned=0")).cnt;
        const td = await db.get("SELECT COUNT(*) as cnt, SUM(amount) as total FROM deposits WHERE status='approved'");
        const pd = (await db.get("SELECT COUNT(*) as cnt FROM deposits WHERE status='pending'")).cnt;
        const tw = await db.get("SELECT COUNT(*) as cnt, SUM(amount) as total FROM withdrawals WHERE status='approved'");
        const pw = (await db.get("SELECT COUNT(*) as cnt FROM withdrawals WHERE status='pending'")).cnt;
        const total_orders = (await db.get("SELECT COUNT(*) as cnt FROM orders WHERE status='COMPLETED'")).cnt;
        const today = new Date().toISOString().slice(0,10);
        const revenue_today = (await db.get("SELECT SUM(charged_price) as rev FROM orders WHERE status='COMPLETED' AND created_at >= ?", today)).rev || 0;
        return {
            total_users,
    ...
    async getOrderById(orderId) {
        return db.get("SELECT * FROM orders WHERE order_id = ? OR id = ?", orderId, orderId);
    },
    async getUsername(telegram_id) {
        const row = await db.get("SELECT username FROM users WHERE telegram_id = ?", telegram_id);
        return row ? row.username : null;
    },
    async getAllUsers() {
        const rows = await db.all("SELECT telegram_id FROM users WHERE banned = 0");
        return rows.map(r => r.telegram_id);
    },
    async createReferral(referrer_id, referred_id) {
        try {
            const reward = REFERRAL_REWARD;
            await db.run(`INSERT INTO referrals (referrer_id, referred_id, status, created_at)
                          VALUES (?, ?, 'completed', datetime('now'))`, referrer_id, referred_id);
            await db.run(`UPDATE users SET balance = balance + ?, referral_balance = referral_balance + ? WHERE telegram_id = ?`, reward, reward, referrer_id);
            await db.run(`UPDATE referrals SET reward_given = 1, reward_amount = ?, rewarded_at = datetime('now') WHERE referred_id = ?`, reward, referred_id);
            return true;
        } catch (e) {
            return false;
        }
    ...
    async getReferralStats(user_id) {
        const total = (await db.get("SELECT COUNT(*) as cnt FROM referrals WHERE referrer_id = ?", user_id)).cnt;
        const rewarded = (await db.get("SELECT COUNT(*) as cnt FROM referrals WHERE referrer_id = ? AND reward_given = 1", user_id)).cnt;
        const total_reward = (await db.get("SELECT SUM(reward_amount) as total FROM referrals WHERE referrer_id = ? AND reward_given = 1", user_id)).total || 0;
        return { total, rewarded, total_reward };
    },
    async getReferralList(user_id, limit = 20) {
        return db.all("SELECT referred_id, status, reward_given, created_at FROM referrals WHERE referrer_id = ? ORDER BY id DESC LIMIT ?", user_id, limit);
    },
    async createPromoCode(code, amount, max_uses = 1, expires_days = 365, user_restricted_id = 0) {
        ...
        try {
            await db.run(`INSERT INTO promo_codes (code, amount, max_uses, used_count, expires_at, user_restricted_id)
    ...
    async getPromoCode(code) {
        return db.get("SELECT * FROM promo_codes WHERE code = ?", code.toUpperCase());
    },
    async hasUserUsedCode(code, user_id) {
        const row = await db.get("SELECT 1 FROM promo_code_uses WHERE code = ? AND user_id = ?", code.toUpperCase(), user_id);
        return !!row;
    },
    async recordPromoCodeUse(code, user_id) {
        await db.run("INSERT INTO promo_code_uses (code, user_id, used_at) VALUES (?, ?, datetime('now'))", code.toUpperCase(), user_id);
    },
    async usePromoCode(code, user_id) {
        ...
        await db.run("UPDATE promo_codes SET used_count = used_count + 1 WHERE code = ?", code.toUpperCase());
        await this.recordPromoCodeUse(code, user_id);
        return { success: true, amount: promo.amount, msg: "" };
    },
    async deletePromoCode(code) {
        await db.run("DELETE FROM promo_code_uses WHERE code = ?", code.toUpperCase());
        await db.run("DELETE FROM promo_codes WHERE code = ?", code.toUpperCase());
    },
    async listPromoCodes() {
        return db.all("SELECT * FROM promo_codes ORDER BY amount DESC");
    },
    async getProductPriceOverride(product_id) {
        const row = await db.get("SELECT price_override FROM product_prices WHERE product_id = ?", product_id);
        return row ? row.price_override : null;
    },
    async setProductPriceOverride(product_id, price, type = 'stars') {
        if (price === null || price <= 0) {
            await db.run("DELETE FROM product_prices WHERE product_id = ?", product_id);
        } else {
            await db.run(`INSERT OR REPLACE INTO product_prices (product_id, game_code, type, price_override, updated_at)
                          VALUES (?, 'Telegram', ?, ?, datetime('now'))`, product_id, type, price);
        }
    },
    async getAllProductOverrides() {
        return db.all("SELECT * FROM product_prices ORDER BY type, product_id");
    },
    async isTransactionUsed(txid) {
        const row = await db.get("SELECT 1 FROM used_transactions WHERE transaction_id = ?", txid.toUpperCase());
        return !!row;
    },
    async recordTransactionUse(txid, user_id, amount) {
        await db.run("INSERT INTO used_transactions (transaction_id, user_id, amount, created_at) VALUES (?, ?, ?, datetime('now'))", txid.toUpperCase(), user_id, amount);
    }
};
...
function mainKeyboard() {
    return Markup.inlineKeyboard([
        [Markup.button.callback('Profile', 'menu_profile'), Markup.button.callback('Service', 'menu_service')],
        [Markup.button.callback('Deposit', 'menu_deposit')],
        [Markup.button.callback('My Orders', 'menu_orders'), Markup.button.callback('Withdraw', 'menu_withdraw')],
        [Markup.button.callback('Support', 'menu_support')]
    ]);
}
function profileKeyboard() {
    return Markup.inlineKeyboard([
        [Markup.button.callback('My Profile', 'profile_show'), Markup.button.callback('Referral', 'profile_referral')],
        [Markup.button.callback('Redeem', 'profile_redeem')],
        [Markup.button.callback('Back to Main', 'back_to_main')]
    ]);
}
function serviceKeyboard() {
    return Markup.inlineKeyboard([
        [Markup.button.callback('Telegram Stars', 'svc_telegram_stars')],
        [Markup.button.callback('Telegram Premium', 'svc_telegram_premium')],
        [Markup.button.callback('Back to main menu', 'back_to_main')]
    ]);
}
function confirmKeyboard() {
    return Markup.inlineKeyboard([
        [Markup.button.callback('Confirm', 'order_confirm'), Markup.button.callback('Cancel', 'order_cancel')],
        [Markup.button.callback('Back', 'order_back')]
    ]);
}
function depositKeyboard() {
    return Markup.inlineKeyboard([
        [Markup.button.callback('Telebirr (ETB)', 'dep_method:telebirr'), Markup.button.callback('CBE (ETB)', 'dep_method:cbe')],
        [Markup.button.callback('Cancel', 'cancel_action')]
    ]);
}
function withdrawKeyboard() {
    return Markup.inlineKeyboard([
        [Markup.button.callback('Telebirr (ETB)', 'withdraw_method:telebirr')],
        [Markup.button.callback('Cancel', 'cancel_action')]
    ]);
}
function supportKeyboard() {
    const buttons = [
        [Markup.button.url('Contact Admin', 'https://t.me/el_bicho14'), Markup.button.url('Updates Channel', `https://t.me/${config.UPDATES_CHANNEL}`)]
    ];
    if (config.SUPPORT_WEBSITE) {
        buttons.push([Markup.button.url('Visit Website', config.SUPPORT_WEBSITE)]);
    }
...
function adminKeyboard() {
    return Markup.inlineKeyboard([
        ...
        [Markup.button.callback('Promo Codes', 'admin_promo')],
        [Markup.button.callback('Referral Lookup', 'admin_referral')],
        [Markup.button.callback('Search by ID', 'admin_search_by_id')],
        [Markup.button.callback('Settings & Tools', 'admin_settings')],
        [Markup.button.callback('Broadcast', 'admin_broadcast')],
        [Markup.button.callback('Close', 'admin_close')]
    ]);
}
function adminPromoKeyboard() {
    return Markup.inlineKeyboard([
        [Markup.button.callback('Create Code', 'admin_promo_create')],
        [Markup.button.callback('List Codes', 'admin_promo_list')],
        [Markup.button.callback('Delete Code', 'admin_promo_delete')],
        [Markup.button.callback('Back', 'admin_back')]
    ]);
}
function adminSettingsKeyboard() {
    return Markup.inlineKeyboard([
        [Markup.button.callback('User Management', 'admin_user_manage')],
        [Markup.button.callback('Telegram Stars Markup', 'admin_stars_markup')],
        [Markup.button.callback('Telegram Premium Markup', 'admin_premium_markup')],
        [Markup.button.callback('Set Product Price', 'admin_set_product_price')],
        [Markup.button.callback('Toggle Maintenance', 'admin_toggle_maintenance')],
        [Markup.button.callback('Toggle Reports', 'admin_toggle_reports')],
        [Markup.button.callback('Back', 'admin_back')]
    ]);
}
function userManageKeyboard() {
    return Markup.inlineKeyboard([
        [Markup.button.callback('Ban User', 'admin_ban')],
        [Markup.button.callback('Unban User', 'admin_unban')],
        [Markup.button.callback('Set Balance', 'admin_set_balance')],
        [Markup.button.callback('Back', 'admin_settings')]
    ]);
}
function searchByIdKeyboard() {
    return Markup.inlineKeyboard([
        [Markup.button.callback('Order', 'admin_search_id:order')],
        [Markup.button.callback('Deposit', 'admin_search_id:deposit')],
        [Markup.button.callback('Withdrawal', 'admin_search_id:withdrawal')],
        [Markup.button.callback('Back', 'admin_back')]
    ]);
}
function cancelKeyboard() {
    return Markup.inlineKeyboard([[Markup.button.callback('Cancel', 'cancel_action')]]);
}

// =========================== CATALOG SERVICE ===========================
class CatalogService {
    constructor() {
        this._telegramCode = null;
    }
    ...
    async getTelegramStarsMarkup() {
        return TELEGRAM_STARS_MARKUP;
    }
    async getTelegramPremiumMarkup() {
        return TELEGRAM_PREMIUM_MARKUP;
    }
}
...
bot.on('text', async (ctx) => {
    ...
    // --- ADMIN GENERAL DECLINE REASON (handled elsewhere) ---
    // (We'll handle it through a separate message handler for replies)
    // skip for now
...
// =========================== ADMIN PANEL ===========================
// Admin action handlers – all start with 'admin_'
...
// =========================== LEGACY COMMANDS (broadcast, gencode, listcodes, delcode, refer, listgames) ===========================
...
(async () => {
    ...
    const settings = await db.all("SELECT key FROM settings");
